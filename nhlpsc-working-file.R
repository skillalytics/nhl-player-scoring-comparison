...
# Add Adjustment Factors to skater_stats_nontot and Calculate Adj Stats
skater_stats_nontot <- merge(
  skater_stats_nontot, season_dat[ , c('SEASON', 'ADJFAC_GPG', 'ADJFAC_APG', 'ADJFAC_PTSPG')], by='SEASON', all.x = TRUE
  )
...
career_adj_stats <- skater_stats_nontot %>%
  group_by(PLAYER_NAME) %>%
  summarise(
...
career_stats_adj <- merge(
  career_stats, career_adj_stats, by='PLAYER_NAME', all = TRUE
  )

# Sort by Leaders
  #GPG
  career_stats_adj <- subset(career_stats_adj, G_TOT >= 100)
  career_stats_adj <- career_stats_adj[order(-career_stats_adj$AVG_ADJ_GPG),]
  rownames(career_stats_adj) <- 1:nrow(career_stats_adj)
  #APG
  career_stats_adj <- career_stats_adj[order(-career_stats_adj$AVG_ADJ_APG),]
  #PTSPG
  career_stats_adj <- career_stats_adj[order(-career_stats_adj$AVG_ADJ_PTSPG),]
...
# their high scoring leads to a high GPG for that season which causes the adjustment factor to adjust downward based on the baseline, therefore bringing down their GPG in that season
  ...
  skater_stats_non100cg <- skater_stats_nontot[skater_stats_nontot$PLAYER_NAME %notin% career_100G_list$PLAYER_NAME, ]
  ...
  skater_stats_incld100cg <- merge(
    skater_stats_nontot, season_dat_non100cg[ , c('SEASON', 'ADJFAC_GPG', 'ADJFAC_APG', 'ADJFAC_PTSPG')], by='SEASON', all.x = TRUE
  )
  skater_stats_incld100cg <- skater_stats_incld100cg[-c(13:18)]
  colnames(skater_stats_incld100cg)[10:12] <- c("ADJFAC_GPG", "ADJFAC_APG", "ADJFAC_PTSPG")
  ...
  career_stats_incld100cg <- merge(
    career_stats_adj, career_adj_stats_incld100cg, by='PLAYER_NAME', all = TRUE
  )
...
# next we will try to remove only the goals each season from the top scorers
  ...
  ggplot(skater_stats_nontot, aes(x=SEASON, y=G)) + geom_point()
  ggplot(skater_stats_nontot, aes(x=SEASON, y=GPG)) + geom_point()
  
  ggplot(skater_stats_nontot, aes(x=SEASON, y=A)) + geom_point()
  
  ggplot(skater_stats_nontot, aes(x=SEASON, y=PTS)) + geom_point()
  
  # Locate and Remove Outliers
  
    # Goals
    g_outlier_tbl <- skater_stats_nontot %>%
      select(SEASON, G) %>%
      group_by(SEASON) %>%
      summarise(
        MEAN = mean(G),
    ...
    skater_stats_goal_outliers_removed <- merge(skater_stats_nontot, g_outlier_tbl, by='SEASON', all=TRUE)
    ...
    # Assists
    a_outlier_tbl <- skater_stats_nontot %>%
      select(SEASON, A) %>%
      group_by(SEASON) %>%
      summarise(
        MEAN = mean(A),
    ...
    skater_stats_assist_outliers_removed <- merge(skater_stats_nontot, a_outlier_tbl, by='SEASON', all=TRUE)
    ...
    # Points
    pts_outlier_tbl <- skater_stats_nontot %>%
      select(SEASON, PTS) %>%
      group_by(SEASON) %>%
      summarise(
        MEAN = mean(PTS),
    ...
    skater_stats_point_outliers_removed <- merge(skater_stats_nontot, pts_outlier_tbl, by='SEASON', all=TRUE)
  ...
  # Add Adjustment Factors to skater_stats and Calculate Adj Stats
    ...
    skater_stats_goal_outliers_removed <- merge(
      skater_stats_nontot, season_dat_goal_outliers_removed[ , c('SEASON', 'ADJFAC_GPG')], by='SEASON', all.x = TRUE
    )
    ...
    skater_stats_assist_outliers_removed <- merge(
      skater_stats_nontot, season_dat_assist_outliers_removed[ , c('SEASON', 'ADJFAC_APG')], by='SEASON', all.x = TRUE
    )
    ...
    skater_stats_point_outliers_removed <- merge(
      skater_stats_nontot, season_dat_point_outliers_removed[ , c('SEASON', 'ADJFAC_PTSPG')], by='SEASON', all.x = TRUE
    )
    ...
    #---  Adjust Goals per Season based on Adj_GPG by Season and Sum for Career Totals  ---#
    skater_stats_goal_outliers_removed$ADJ_G <- skater_stats_goal_outliers_removed$GP*skater_stats_goal_outliers_removed$ADJ_GPG
    ...
    #---  Adjust with removal of individual player goals for that season  ---#
    skater_stats_goal_outliers_removed2 <- merge(
      skater_stats, season_dat_goal_outliers_removed[ , c('SEASON', 'ADJFAC_GPG')], by='SEASON', all.x = TRUE
    )
    
    skater_stats_goal_outliers_removed2 <- skater_stats_goal_outliers_removed2 %>%
      mutate(
        ADJ_GPG = GPG*ADJFAC_GPG
      )
    
    skater_stats_goal_outliers_removed2 <- merge(
      skater_stats_goal_outliers_removed2, season_dat[ , c('SEASON', 'G_TOT', 'LEAG_GP')], by='SEASON', all.x = TRUE
    )
    ...
    skater_stats_goal_outliers_removed2 <- skater_stats_goal_outliers_removed2 %>%
      mutate(
        ...
        ADJFAC_GPG2 = BASELINE_GPG/LEAG_GPG_LESSCP,
        ADJ_GPG2 = GPG*ADJFAC_GPG2,
        ADJ_G2 = GP*ADJ_GPG2
      )
    
    career_adj_stats_goal_outliers_removed2 <- skater_stats_goal_outliers_removed2 %>%
      group_by(PLAYER_NAME) %>%
      summarise(
        GP = sum(GP),
        G = sum(G),
        AVG_ADJ_GPG = mean(ADJ_GPG2),
        ADJ_G2 = sum(ADJ_G2)
      ) %>%
      mutate(
        ADJ_AVG_GPG2 = ADJ_G2/GP
      )
    
    
    #---  Adjust Assists per Season based on Adj_APG by Season and Sum for Career Totals  ---#
    skater_stats_assist_outliers_removed$ADJ_A <- skater_stats_assist_outliers_removed$GP*skater_stats_assist_outliers_removed$ADJ_APG
    ...
    #---  Adjust with removal of individual player assists for that season  ---#
    skater_stats_assist_outliers_removed2 <- merge(
      skater_stats_assist_outliers_removed, season_dat[ , c('SEASON', 'A_TOT', 'LEAG_GP')], by='SEASON', all.x = TRUE
    )
    
    BASELINE_APG <- sum(season_dat$A_TOT)/sum(season_dat$LEAG_GP)
    
    skater_stats_assist_outliers_removed2 <- skater_stats_assist_outliers_removed2 %>%
      mutate(
        LEAG_APG_LESSCP = (A_TOT - A)/LEAG_GP,
        ADJFAC_APG2 = BASELINE_APG/LEAG_APG_LESSCP,
        ADJ_APG2 = APG*ADJFAC_APG2,
        ADJ_A2 = GP*ADJ_APG2
      )
    
    career_adj_stats_assist_outliers_removed2 <- skater_stats_assist_outliers_removed2 %>%
      group_by(PLAYER_NAME) %>%
      summarise(
        GP = sum(GP),
        A = sum(A),
        AVG_ADJ_APG = mean(ADJ_APG2),
        ADJ_A2 = sum(ADJ_A2)
      ) %>%
      mutate(
        ADJ_AVG_APG2 = ADJ_A2/GP
      )
    
    #---  Adjust Points per Season based on Adj_APG by Season and Sum for Career Totals  ---#
    skater_stats_point_outliers_removed$ADJ_PTS <- skater_stats_point_outliers_removed$GP*skater_stats_point_outliers_removed$ADJ_PTSPG
    ...
    #---  Adjust with removal of individual player assists for that season  ---#
    skater_stats_point_outliers_removed2 <- merge(
      skater_stats_point_outliers_removed, season_dat[ , c('SEASON', 'PTS_TOT', 'LEAG_GP')], by='SEASON', all.x = TRUE
    )
    
    BASELINE_PTSPG <- sum(season_dat$PTS_TOT)/sum(season_dat$LEAG_GP)
    
    skater_stats_point_outliers_removed2 <- skater_stats_point_outliers_removed2 %>%
      mutate(
        LEAG_PTSPG_LESSCP = (PTS_TOT - PTS)/LEAG_GP,
        ADJFAC_PTSPG2 = BASELINE_PTSPG/LEAG_PTSPG_LESSCP,
        ADJ_PTSPG2 = PTSPG*ADJFAC_PTSPG2,
        ADJ_PTS2 = GP*ADJ_PTSPG2
      )
    
    career_adj_stats_point_outliers_removed2 <- skater_stats_point_outliers_removed2 %>%
      group_by(PLAYER_NAME) %>%
      summarise(
        GP = sum(GP),
        PTS = sum(PTS),
        AVG_ADJ_PTSPG = mean(ADJ_PTSPG2),
        ADJ_PTS2 = sum(ADJ_PTS2)
      ) %>%
      mutate(
        ADJ_AVG_PTSPG2 = ADJ_PTS2/GP
      )
